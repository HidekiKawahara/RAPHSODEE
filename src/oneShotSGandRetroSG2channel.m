...
outputSG = basicPlayRecLoop(fs,y,devices{devId});
...
ltiSGRF = fft(iRespSG(headLTI+(1:lIresp),:),length(xOrg));
rtvSGRF = fft(iRespSG(round(nData/2)+(1:lIresp),:),length(xOrg));
...
set(gca,"FontSize",13,"LineWidth",2)
axis([20 fs/2 [-70 10] + max(ltiSG(:))])
xlabel("Frequency (Hz)")
ylabel("level (dB)")
legend("Ch1 LTI","Ch2 LTI","Ch1 RTV","Ch2 RTV","Location","northwest")
...
xline(highLimit,"LineWidth",2,"Color",'g');
set(gca,"FontSize",13,"LineWidth",2)
axis([20 fs/2 [-70 10] + max(ltiRawSG(:))])
xlabel("Frequency (Hz)")
ylabel("level (dB)")
legend("Ch1 LTI","Ch2 LTI","Ch1 RTV","Ch2 RTV","Location","northwest")
...
maxGain = max(ltiRawSG(fxSG>lowLimit & fxSG < highLimit,1)- ...
    ltiRawSG(fxSG>lowLimit & fxSG < highLimit,2));
figure;
semilogx(fxSG,ltiRawSG(:,1)-ltiRawSG(:,2)-maxGain,"LineWidth",2);grid on;
hold on;semilogx(fxSG,ltiSG(:,1)-ltiSG(:,2)-maxGain,"LineWidth",2);grid on;
axis([20 fs/2 -60 20])
xline(lowLimit,"LineWidth",2,"Color",'g');
xline(highLimit,"LineWidth",2,"Color",'g');
set(gca,"FontSize",13,"LineWidth",2)
